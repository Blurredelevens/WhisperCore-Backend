...
class MemoryListAPI(MethodView):
    ...
    def get(self):
        ...
        if search_query:
            ...
            memories = [memory.to_dict_with_keys(key, model_key) for memory in paginated_memories]
        ...
        if group_by_chat_id:
            ...
            for memory in all_memories:
                ...
                grouped_memories[chat_id_key]['memories'].append(
                    memory.to_dict_with_keys(key, model_key)
                )
                total_memories += 1
        ...
        memories = [memory.to_dict_with_keys(key, model_key) for memory in pagination.items]
    ...
    def post(self):
        try:
            ...
            return jsonify({'memory': memory.to_dict_with_keys(key, model_key)}), 201
        except Exception as e:
...
class MemoryDetailAPI(MethodView):
    ...
    def get(self, memory_id):
        ...
        return jsonify({'memory': memory.to_dict_with_keys(key, model_key)}), 200
    ...
    def put(self, memory_id):
        ...
        db.session.commit()
        return jsonify({'message': 'Memory updated successfully', 'memory': memory.to_dict_with_keys(key, model_key)}), 200
...
memory_bp.add_url_rule('/chats', view_func=MemoryChatListAPI.as_view('memory_chat_list'))
