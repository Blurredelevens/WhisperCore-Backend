...
def prompt(db_session, user):
    ...
    prompt = Prompt(
        text="What are your thoughts on today?",
        is_active=True,
        user_id=user.id
    )
...
def mock_llm_service(monkeypatch):
    ...
    class MockLLMService:
        def process_query(self, query):
            return {"data": "Mock LLM response"}
    
    monkeypatch.setattr("tasks.llm_service.LLMService", MockLLMService)
...
def sample_audio_base64():
    ...
    return "UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBj+a2/LDciUFLIHO8tiJNwgZaLvt559NEAxQp+PwtmMcBjiR1/LMeSwFJHfH8N2QQAoUXrTp66hVFApGn+DyvmwhBSuBzvLZiTYIG2m99OScTgwOUarm7blmGgU7k9n1unEiBC13yO/eizEIHWq+9+OWT"  # Truncated for brevity
