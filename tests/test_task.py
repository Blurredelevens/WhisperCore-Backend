...
class TestTaskAPI:
    ...
    @patch('routes.task.celery')
    def test_create_task_with_chat_id_and_mood_emoji(self, mock_celery, client, db_session, auth_headers, user):
        ...
        memory = Memory.query.get(result["memory_id"])
        assert memory is not None
        assert memory.chat_id == "test_chat_123"
        assert memory.mood_emoji == "😊"
    
    @patch('routes.task.celery')
    def test_create_task_without_chat_id_and_mood_emoji(self, mock_celery, client, db_session, auth_headers, user):
        ...
        data = {
            "content": "Test task content without chat_id"
        }
        ...
        assert result["message"] == "Task started successfully"
        assert result["status"] == "processing"
        assert result["task_id"] == "test-task-id-456"
        assert result["chat_id"] is None
        assert result["mood_emoji"] is None
        assert "memory_id" in result
