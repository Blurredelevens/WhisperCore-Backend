...
class TestPromptDetailAPI:
    ...
    def test_delete_prompt_admin_success(self, client, db_session, admin_auth_headers, admin_user):
        ...
        deleted_prompt = Prompt.query.get(prompt.id)
...
class TestTodayPromptAPI:
    """Test cases for today's prompt API."""
    
    @patch('routes.prompt.redis_client')
    def test_get_today_prompt_success(self, mock_redis, client, db_session, auth_headers):
        """Test successful retrieval of today's prompt from Redis."""
        mock_redis.get.return_value = b"Today's reflection prompt"
        ...
        assert response.status_code == 200
        result = json.loads(response.data)
        assert result["prompt"] == "Today's reflection prompt"
    
    @patch('routes.prompt.redis_client')
    def test_get_today_prompt_not_found(self, mock_redis, client, db_session, auth_headers):
        """Test when no prompt is set for today."""
